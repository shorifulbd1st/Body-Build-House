...
const AddNewSlots = () => {
  ...
  const [selectedDays, setSelectedDays] = useState([]);
  useEffect(() => {
    const days = colourOptions.filter((option) =>
  ...
  const handleSubmitData = async (e) => {
    ...
    if (slotTime > availableTime) {
      notify(
        "error",
        "Your available time is grater than slot Time. You cannot book any more slots."
      );
      navigate("/");
    }
    ...
    <div className="w-11/12 mx-auto my-5 border-2 border-blue-500 rounded-xl">
      <div className=" p-4 bg-white shadow-md rounded-lg">
        ...
        <form onSubmit={handleSubmitData}>
          ...
          <div>
            <p className="border-b-2  border-red-600 my-5"></p>
          </div>
